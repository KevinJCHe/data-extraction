...
def get_recipe_data(recipe_search, conn, header):
	...
	for r in range(len(recipe_search)):				#GET API KEY FROM FOOD2FORK. ITS FREE
		...
		#Saves the key values in python variables
		...
		#GET RECIPE IAMGE URL AND INGREDIENTS
...
def get_only_ingredient(ing_list):	#MAKE SURE ing_list is passed along as str(...), not unicode!
	...
	for ingr in ing_list:
		...
		if ing.endswith(r'\n' + " " + " "):
			ing = ing[:-4]							#Remove that //n at the end							#Remove that //n at the end
		...
		ing = ing.strip().strip("-")				#Remove extra spacing and hyphen in front/back of word. Strip spacing FIRST!!
